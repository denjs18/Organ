...
const TECHNIQUES = [
  ...
  },
  {
    ...
    why: "Savoir harmoniser libère l'improvisateur : vous pouvez jouer n'importe quelle mélodie connue (cantique, thème liturgique) et lui ajouter une harmonisation personnelle et expressives.",
    rules: [
  ...
  },
  {
    ...
    why: "Varier la texture d'un accord permet de maintenir l'intérêt tout au long d'une pièce, de graduer l'intensité (commencer doucement, arriver au plein), et d'adapter le style à chaque moment liturgique.",
    textures: [
      {
        nom: 'Accords plaqués',
        notation: '♩ ♩ ♩ ♩',
        desc: "Toutes les notes jouées ensemble sur chaque temps. Noble, direct, affirmé.",
        usage: "Entrées, acclmations, sorties festives.",
        midi: [60, 64, 67],
      },
      {
        nom: 'Arpège',
...
function TechniqueContent({ technique }) {
  switch (technique.id) {
    case 'basse-obstinee':
      return <BasseObstineeContent t={technique} />
    case 'pedal-point':
      return <PedalPointContent t={technique} />
    case 'ornements':
      return <OrnementContent t={technique} />
    case 'contrepoint':
      return <ContrepointContent t={technique} />
    case 'modulation':
      return <ModulationContent t={technique} />
    case 'harmonisation':
      return <HarmonisationContent t={technique} />
    case 'ostinato-rythmique':
      return <OstinatoRythmiqueContent t={technique} />
    default:
      return null
  }
}

// ─── Individual technique content ─────────────────────────────────────────────
          ...
          {t.ornements.map((o) => (
            <div key={o.nom} className="bg-[#0f0f1a] rounded-xl p-4 border border-[#2a2a4a]">
              ...
              <p className="text-gray-400 text-xs mb-1">
                <span className="text-gray-500">Doigté : </span>{o.fingering}
              </p>
              <p className="text-gray-400 text-xs">
                <span className="text-gray-500">Utilisation : </span>{o.usage}
              </p>
            </div>
          ))}
        </div>
          ...
          {t.modulations.map((m) => (
            <div key={m.type} className="bg-[#0f0f1a] rounded-xl p-4 border border-[#2a2a4a]">
              ...
              <p className="text-gray-300 text-xs mb-1">
                <span className="text-gray-500">Exemple : </span>{m.exemple}
              </p>
              <p className="text-gray-400 text-xs mb-1">
                <span className="text-gray-500">Pivot : </span>{m.pivot}
              </p>
              <p className="text-gray-400 text-xs">
                <span className="text-gray-500">Effet : </span>{m.effet}
              </p>
            </div>
          ))}
        </div>
