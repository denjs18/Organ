...
function detectPeriode() {
  ...
  // Avent : 4 dimanches avant Noël — approximation : ~1er déc au 24 déc
  ...
  // Carême : 46 jours avant Pâques (Mercredi des Cendres) jusqu'à Samedi Saint
  const cendres = easterDay - 46
  if (dayOfYear >= cendres && dayOfYear < easterDay) return 'careme'

  // Pâques : Pâques – Pentecôte (50 jours après)
...
function getMomentAdvice(moment, occasionId, periodeId) {
  ...
  const isAvent = periodeId === 'avent'

  // ── COMMUNION (always soft) ──
  if (m.includes('communion') || m.includes('méditation après')) {
    if (occasionId === 'funerailles') {
      return {
  ...
  if (m.includes('prélude') || m.includes('prelude')) {
    ...
    if (isFestif || occasionId === 'mariage' || occasionId === 'profession-foi') {
      return {
  ...
  if (m.includes('sortie') || m.includes('postlude') || m.includes('envoi')) {
    ...
    if (occasionId === 'messe-scolaire' || occasionId === 'bapteme') {
      return {
  ...
  // ── PSAUME ──
  if (m.includes('psaume') || m.includes('liturgie de la parole')) {
    return {
  ...
  // ── APRÈS LE BAPTÊME ──
  if (m.includes('après le baptême') || m.includes('notre père')) {
    return {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {selectedOccasion && (
          <div className="mt-2 flex flex-wrap gap-1">
            {selectedOccasion.moments.map(m => (
              <span
                key={m}
                className="text-xs px-2 py-0.5 rounded-full bg-[#1a1a2e] border border-[#2a2a4a] text-gray-500"
              >
                {m}
              </span>
            ))}
      ...
      <div>
        <label className="block text-xs font-semibold uppercase tracking-wide text-gray-400 mb-2">
          Période liturgique
          {periodeId === detectedPeriode && (
            <span className="ml-2 text-[#d4a017] normal-case font-normal">
          ...
          {PERIODES.map(p => (
            <button
              ...
              className={`px-3 py-2 rounded-xl border text-sm font-medium transition-all ${
                periodeId === p.id
                  ? 'text-white'
                  : 'bg-[#1a1a2e] border-[#2a2a4a] text-gray-300 hover:bg-[#1e1e35] hover:border-[#3a3a6a]'
              }`}
              style={
                periodeId === p.id
      ...
      <div>
        ...
        <input
          ...
          max={8}
          value={nbCantiques}
          onChange={e => setNbCantiques(Number(e.target.value))}
          className="w-full"
        />
  ...
  return (
    <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-xl overflow-hidden">
      ...
      <div
        ...
        style={{ borderLeftWidth: 4, borderLeftColor: periodeColor }}
      >
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ borderColor: periode.color, backgroundColor: `${periode.color}10` }}
      >
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm font-semibold mb-1" style={{ color: periode.color }}>
              {periode.name}
            </p>
          ...
          <div className="text-right text-sm text-gray-400 space-y-1">
            ...
            <div>
              ...
              <p className="font-semibold" style={{ color: periode.color }}>
                {suggestedKey.keys.join(' / ')}
              </p>
      ...
      <div className="space-y-4 mb-6">
        {occasion.moments.map((moment, idx) => (
          <MomentCard
            ...
            periodeColor={periode.color}
            noteValue={notes[idx] || ''}
            onNoteChange={val => updateNote(idx, val)}
          />
        ))}
      </div>
          ...
          <div className="space-y-2">
            {Array.from({ length: nbCantiques }).map((_, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <input
                  type="text"
                  placeholder={`Cantique ${i + 1} — titre et tonalité`}
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 fade-in-up">
      ...
      <div className="bg-[#1a1a2e] border border-[#2a2a4a] rounded-xl p-5">
        {plan === null ? (
          <PreparationForm onSubmit={setPlan} />
        ) : (
          <PlanView config={plan} onReset={() => setPlan(null)} />
        )}
      </div>
    </div>
  )
